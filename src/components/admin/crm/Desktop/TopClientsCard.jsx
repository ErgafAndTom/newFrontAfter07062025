...
const TopClientsCard = ({ data }) => {
    if (!data || data.length === 0) {
        return (
            <div className="dsh-card" style={{ overflow: 'hidden' }}>
                <div className="dsh-card-title">Карта клієнтів</div>
                <div style={{ color: 'var(--admingrey)', opacity: 0.5, padding: '1rem', textAlign: 'center', fontSize: '0.7vw' }}>
                    Немає даних
                </div>
            </div>
        );
    ...
    return (
        <div className="dsh-card" style={{ overflow: 'hidden', padding: 0 }}>
            <div className="dsh-card-title" style={{ padding: '0.8rem 1rem 0.4rem' }}>Карта клієнтів</div>
