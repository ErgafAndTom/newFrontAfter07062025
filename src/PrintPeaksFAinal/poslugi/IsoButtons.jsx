...
export default function PaperPreview({ sizeOfPaper = {x: 310, y: 440}, size, material = {material: "SR A3(ghost)"} }) {
  if (!sizeOfPaper?.x || !sizeOfPaper?.y) return null;

  const paperAspect = sizeOfPaper.y / sizeOfPaper.x;

  const productKx =
    size?.x && sizeOfPaper?.x ? size.x / sizeOfPaper.x : 0;
  const productKy =
    size?.y && sizeOfPaper?.y ? size.y / sizeOfPaper.y : 0;
    ...
    <div style={{
      ...
      <div
        style={{
          // marginTop: "6vh",
          width: "15vw",
          aspectRatio: `${sizeOfPaper.x} / ${sizeOfPaper.y}`,
      ...
      >
        <div style={{
          ...
          textAlign: "center",
          whiteSpace: "nowrap", // опционально, если не нужен перенос
          fontSize: "0.5vw",
        }}>
          <div>
            выбран лист:
          </div>
          {material.material} ({sizeOfPaper.x}x{sizeOfPaper.y})
        </div>
